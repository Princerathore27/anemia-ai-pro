...
# --- CLI args ---
parser = argparse.ArgumentParser(description='Evaluate ensemble DenseNet+VGG+Inception on held-out test set')
...
parser.add_argument('--vgg-model',       default='best_vgg16_fold_1.keras',    help='Path to VGG model')
...
try:
    for p in (args.densenet_model, args.vgg_model, args.inception_model):
        if not os.path.isfile(p):
            raise FileNotFoundError(p)
    model_densenet  = tf.keras.models.load_model(args.densenet_model)
    model_vgg16     = tf.keras.models.load_model(args.vgg_model)
    model_inception = tf.keras.models.load_model(args.inception_model)
    print("All three models loaded successfully!")
except Exception as e:
    print(f"Error loading models: {e}")
    print("Please make sure you have run all three K-Fold scripts first.")
    sys.exit(1)

# --- 3. Separate Data Generators (each model has its own preprocessing + image size) ---
...
# --- 6. Evaluate ---
print("\n--- Final Ensemble Performance Report (DenseNet121 + VGG16 + InceptionV3) ---")
...
out_cm = os.path.join(args.output_dir, 'confusion_matrix_final_ensemble.png')
plt.savefig(out_cm)
