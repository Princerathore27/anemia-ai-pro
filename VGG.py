...
parser = argparse.ArgumentParser(description='Train VGG16 K-Fold on anemia dataset')
...
for train_index, test_index in skf.split(X, y):
    ...
    model = create_model()
    checkpoint_path = os.path.join(args.output_dir, f'best_vgg16_fold_{fold_no}.keras')
    ...
    # --- FIX: Unfreeze the last convolutional block (block5) of VGG16 ---
    # VGG16 layers: block5_conv1 is layer index 15 onward
    for layer in model.layers:
        layer.trainable = False
    for layer in model.layers:
        if layer.name.startswith('block5') or layer.name in ('global_average_pooling2d', 'dropout', 'dense'):
...
plt.title('Aggregate Confusion Matrix (VGG16 - All Folds)')
out_cm = os.path.join(args.output_dir, 'confusion_matrix_vgg16.png')
plt.savefig(out_cm)
...
for h in histories:
    # Pad shorter histories with their last value, then ACCUMULATE (not divide here)
...
axes[0].set_title('VGG16 Average Accuracy (K-Fold)')
...
axes[1].set_title('VGG16 Average Loss (K-Fold)')
axes[1].set_xlabel('Epoch')
axes[1].set_ylabel('Loss')
axes[1].legend()

plt.tight_layout()
out_lc = os.path.join(args.output_dir, 'learning_curves_vgg16.png')
plt.savefig(out_lc)
