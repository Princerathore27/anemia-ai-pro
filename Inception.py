...
# FIX: InceptionV3 requires minimum 299x299 input — 224x224 is too small
IMG_SIZE = 299

# --- CLI args & 1. Load the dataset ---
parser = argparse.ArgumentParser(description='Train InceptionV3 K-Fold on anemia dataset')
...
for train_index, test_index in skf.split(X, y):
    ...
    train_generator = train_datagen.flow_from_dataframe(
        ...
        target_size=(IMG_SIZE, IMG_SIZE), batch_size=32, class_mode='binary', shuffle=True
    )
    ...
    checkpoint_path = os.path.join(args.output_dir, f'best_inception_fold_{fold_no}.keras')
    ...
    # Fine-tuning: unfreeze the top two Inception blocks (layer 249+)
...
out_cm = os.path.join(args.output_dir, 'confusion_matrix_inceptionv3.png')
plt.savefig(out_cm)
print(f"\nFinal InceptionV3 confusion matrix plot saved as '{out_cm}'")
...
axes[0].set_title('InceptionV3 Average Accuracy (K-Fold)')
...
axes[1].set_title('InceptionV3 Average Loss (K-Fold)')
axes[1].set_xlabel('Epoch')
axes[1].set_ylabel('Loss')
axes[1].legend()

plt.tight_layout()
out_lc = os.path.join(args.output_dir, 'learning_curves_inceptionv3.png')
plt.savefig(out_lc)
